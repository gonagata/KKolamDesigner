...
// 2012/06/22 以後　version 4.5.3とする　onEditHSym()追加
// 2012/06/12 以後　version 4.5.2とする　H/V Rotate Sym, Shift +G -> only G sets Random Generation
//未完　Animation保存のとき　trce=1の第一が現状、trce=2の第二にするか
//     複雑な制約が必要か？　現行if( animImage && (trce ==1 || m_smoothmod ==2 && trce ==2) )// for saving an animation image
//未完　最後の像色LastLoop、文字は他のWindowの重なりで消える　それ以前の白Loopや文字は消えない
// 2012/06/10 以後　version 4.5.0とする　Edit
//未完　追加　GenerationのSymmetryをHとVに分ける　
...
BEGIN_MESSAGE_MAP(CChildView,CWnd )
	...
	ON_COMMAND(ID_EDIT_H_SYM, OnEditHSym)
	...
	ON_COMMAND(ID_GENERATION_V_SYM, OnGenerationVSym)
	...
	ON_COMMAND(ID_HELPUSAGE, OnHelpUsage)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
unsigned int m_edgeChain[30][140]; // Data[m_fudeNum][m_chnNo[m_fudeNum]Max232] 
// もし１筆で全てのKoMaを巡るchain数= KoMa数(H50xV37=1850)x4/32bits =231.25本, 一本のChainはMSB:start32,LSB:last
...
//		m_fudeNum = m_loopNum -1; 元元なし、これがあるとLoops指定でないときにChainModeが描画なし
		...
		m_memo.Format("");//reset to show Data of Chain Sequence 
...
//		OnKeyUp('K', 0, 0); // to converte a code of  the pattern	
	...
	{
		m_drawMode = 0; SETcnvEc2KoMa = 0;// m_drawMode 0:KoMaTile, 1:EdgeChain, 
	}
...
void CChildView::OnGenerationHvs() // generate Horizonatally Symmetry
{
	// TODO: この位置にコマンド ハンドラ用のコードを追加してください
//	if( fnlimg == 1) fnlimg = 0; // reset 2009/11/28 keeping the finalline image 
	...
	m_gemod = 1; //1:only H-Sym 
	geNeration( m_gemod);// 1:Verti&Hori Sym, 2:RotatedSym, 3:Recursive
//	Invalidate();// into geNeration()
}

void CChildView::OnGenerationVSym() 
{
	// TODO: この位置にコマンド ハンドラ用のコードを追加してください
	for( short ikb = 0; ikb < m_komaNoX*m_komaNoY; ikb++) m_preKoM[ikb] = m_koM[ikb];// for Undo	

	m_gemod = 4; //4:only V-Sym 
...
void CChildView::OnGenerationRot() 
{
	...
	geNeration( m_gemod);// 1:Verti&Hori Sym, 2:RotatedSym, 3:Recursive
	//Invalidate();// move to geNeration( タイル数が少なく描画域が狭くなったとき　前の像を消去
...
//now only order 0,  Random to order Edge data in 0:random,
void CChildView::pattnGenrtn( int order )
{ 	
	short ij, ii; short roop = 1; 

	//孤立のKoMa　即ち　域内でm_koM[　] =0　が出たらやり直し 	
	while(roop)
	{	// set All edges 
		for( short ie = 0; ie < m_ednum; ie++)
		{
			// All edges are connected in random with Prob 0.5
			//if(order == 0) //now only order 0 Random
			//{
				//	if( (double)rand()/(double)RAND_MAX < 0.5)	m_edcd[ie] = 0; 
				//	else										m_edcd[ie] = 1;
			//} // else if(order ==1)  //for Sequence, now moved into geNeration(0) 				
			m_edcd[ie] = rand()%2;// mod(x,2)= 0 or 1
		}

		// make KoMa from Edge-code 
		ie = 0;  // edge index //これを修正したら同じ設定をするgeNeration( )も修正要
		for(ii = 0; ii < m_komaNoY; ii++)
		{ 
			knap = m_komaNoX *ii; ie = (m_komaNoX*2 -1)*ii; // 
			for( ij = 0; ij < m_komaNoX -1; ij++)//横最後のKoMaは不必要
			{       // vertical edges
		...
		//check if KoMa=0 in OriginalArea, So again 
		//検査域 H&VSym Rotleft-upper Quadrant(1/4)　Symmetry Half Side
		short RangeX =m_komaNoX;
		short RangeY =m_komaNoY;
		if( m_gemod == 1){	// Horizontal Symmetry
				//RangeX =m_komaNoX;
				RangeY =(m_komaNoY+1)/2;
		}
		else if( m_gemod == 2){  // Rotated Symmetry
				RangeX =(m_komaNoX+1)/2;
				RangeY =(m_komaNoY+1)/2;
		}
		else if( m_gemod == 3){  // Recursive
		}
		else if( m_gemod == 4){   // Vertical Symmetry 
				RangeX =(m_komaNoX+1)/2;
				//RangeY =(m_komaNoY+1)/2;
		}
		// check all KoMa >0　in Range			
		for( ii = 0; ii < RangeY; ii++) 
		{
			for( ij = 0; ij < RangeX; ij++) // horizontal edges
			{	
				short iikxy = ij + ii *m_komaNoX;
				if( !m_koM[iikxy] ){ roop = 1; break; } //KoMa m_koM[iikxy] = 0 , so again 
				else				 roop = 0;
			}		
			if(roop ==1) break; // stop for( ii = 0; 
		}		
	}// endof while
	//return; // 戻り値がないのだから要らない
}//endof CChildView::ptnGeneration

// Auto-generation of Kolam pattern Mikan !!!!!!!!!!! for 3-status edge
void CChildView::geNeration( int gemodd)// 0:Sequence,1:H-Sym 2:Rot-Sym 3:Recursive 4:V-Sym, Shift +G randomKoMa
{ 
	...
	{	
		case 0:	// countup generating with Edge-code m_edcd[ie]// 31 edges in 5x4 KoMa, 40 edges in 5x5  
				...
				// pim は　m_saveDatafl == 1 データ保存のとき　Multiは無視されない、逆算計数する
			// set all edge 0 or 1 Seqeuncially 
			m_alledge = 0;
			if( pia == 0) //始めに　最後段のみ櫛形接続　pass until the last row of horizontal edges between upper and lower
			{	//　最下部の全横Edgeのみを繋ぐ　そこまでMultiはないので　無視されない　 
			  if( m_komaNoY >1)
			  {
				for( ij = 0; ij < m_komaNoX; ij++) 
				{	// for only the last row Koma connected at upper edge
					ie = m_ednum -m_komaNoX*2 +1 +ij; 
					m_edcd[ie] = 1; //set the last Koma connected at upper edge
					pia = pia + (unsigned)pow(2.0, ie); //発生パターン数の順番
				}				
			  } else if( m_komaNoY ==1)
			  {
				for( ij = 0; ij < m_komaNoX; ij++) 
					ie = 0; 
					m_edcd[ie] = 1; //set first KoMa edge
					pia++; //発生パターン数の順番
			  }
			...
			{	
				...
				if ( m_edcd[ii] == 2) 					
				{	//edgeData m_edcd[]をbit列とみなして、繰上げ（=2）の場合は次のEdge(bit)に+1する。 
					m_edcd[ii] = 0; 
					m_edcd[ii+1]++; 
				}
				pia++;

				//checking if order pia is the last e_edpnm where all edges =1
				//m_alledge = 1;
				//for( ii = 0; ii < m_ednum; ii++) if ( m_edcd[ii]  == 0) { m_alledge = 0;  break; }
				if( pia == m_edpnm) m_alledge = 1; //else m_alledge = 0;// e_edpnm: Max passible digit patterns 
			}
 			// make KoMa from Edge-code 
			ie = 0; // edge index //これを修正したら同じ設定をするpattnGenrtn( int order )も修正要　
			for( ii = 0; ii < m_komaNoY; ii++)
			{
				knap = m_komaNoX *ii;  ie = (m_komaNoX*2 -1)*ii;// 
				for( ij = 0; ij < m_komaNoX -1; ij++)//横最後のKoMaは不必要
				...
				{ // horizontal edges
					...
					knap++; ie++;	// the next/neighbor KoMa
				}

				//未完　ここでしない　なぜ? m_koM[inko] == 0のとき　traceしないように　OnTimer()で設定　　　
			}// endof for(ii
			drwNln = 1; // set Only N-lines withoutGreenDots 
			//Invalidate(FALSE);// show only lastImage without Flash
			break; 
...
/*		case 1:	// left/right & up/down symmetric 
			...
			//Shift +G  set order mode edge data 0 :KoMa random
			//if( GetAsyncKeyState( VK_SHIFT ) & 0x8000 ) 
			//pattnGenrtn(0);//to KeyUp() Only KeyG sets Random   
			...
			// show half original side with Greendot
			...
			break;
*/
		case 1:	// Horizontal left/right  symmetric from left to right side 
			//for( ii = 0; ii < (m_komaNoY+1)/2; ii++) //vertical //left half part
			for( ii = 0; ii < m_komaNoY; ii++) //vertical
			{	
				for( ij = 0; ij < m_komaNoX/2; ij++) //horizontal
				{		
					ie = m_komaNoX -1 -ij + ii *m_komaNoX; // horizontal symmetry in Position
					m_koM[ie] = m_koM[ij + ii *m_komaNoX]; // change
					short e0 = (m_koM[ie]>>0)%2 +((m_koM[ie]>>(4+0))%2) *2; 
					short e2 = (m_koM[ie]>>2)%2 +((m_koM[ie]>>(4+2))%2) *2;
					m_koM[ie] = makeKoma( ie, 0, e2);
					m_koM[ie] = makeKoma( ie, 2, e0);
			...
			LMbX = LMbY = 0;  	// begining KoMa in N-line
			LMeX = (m_komaNoX+1)/2-1;	// ending KoMa in N-line
			LMeY =  m_komaNoY-1;	// Now HalfSide, Old version LMeY = (m_komaNoY+1)/2-1; for Symmfrom upper-left quarter
			reArrang(1); //connect KoMa
			Invalidate(FALSE);// show only the last image nagata
			break;

		case 4:	// only Vertical Symmetry on Upper rows to Lower
			//pattnGenrtn(0); //move to KeyUp() Only KeyG sets random
			...
			for( ij = 0; ij < m_komaNoX; ij++) // horizontal
			{
				for( ii = 0; ii < m_komaNoY/2; ii++) // vertical 
				{		
					ie = ij +(m_komaNoY-1-ii) *m_komaNoX; // vertical symmetry 
					m_koM[ ie ] = m_koM[ij + ii *m_komaNoX]; // change
 					short e1 = (m_koM[ie]>>1)%2 +((m_koM[ie]>>(4+1))%2) *2; 
					short e3 = (m_koM[ie]>>3)%2 +((m_koM[ie]>>(4+3))%2) *2;
					m_koM[ie] = makeKoma( ie, 3, e1);
		...
		case 2:	// Rotated Symmetry
			...
			Invalidate();// show only last image but with flash
			break;
...
//			static deep = 5; 
	...
	prsntedg = -1; 
	//Invalidate(FALSE);//();// show only last image but with flash
	//CChildView::geNeration(gemodd); //selfcalling is wrong, Stock over error 
...
static int chnNum;
static short prvknap; // previous knap, for check if starting KoMa changed in onkeyUp(),onLButtonUp() 
...
//			m_drawImages = TRUE;
			Invalidate(FALSE);// no Flash on changing new image
		break;
		case 'L':// Leading Koma Type step by set to all type or Shift+L to rotate
			//MessageBeep(MB_ICONEXCLAMATION); 	//MessageBeep(0);
			// short int m_N81toE8B[ 0-80;number81]; 0-80 number to 8bit data
			// short int m_E8BtoN81[ 0-240:8bit number 241]; 8bit data to 0-80 number
 			short type[21]; //未完　[06] 以上はもう一度合理的な順にすべき
			type[00] =00; type[01] =01; type[02] =03; type[03] =05; type[04] =07; type[05] =15; 
			type[06] =41; type[07] =44; type[ 8] =53; type[ 9] =27; type[10] =31; type[11] =36;//なぜか[08][09]は拒否される 
			type[12] =20; type[13] =19; type[14] =26; type[15] =23; type[16] =50; type[17] =16; 
			type[18] =17; type[19] =22; type[20] =80; 
			//  17->25, 22->39, 31->40,mirrorなので除いたtype[20] =25;type[23] =39;type[10] =40;
			for( ikb = 0; ikb < m_komaNoX*m_komaNoY; ikb++) m_preKoM[ikb] = m_koM[ikb];// for Undo
			...
			{  // rotation			
				...
				m_koM[knap] = rotateKoma( m_koM[knap]); 
			} else // change the primitive KoMa-cell
			{	//sequentially 
				m_preKoM[knap] = m_koM[knap];
				m_koM[knap] = m_N81toE8B[ type[kmty++] ];	//				
				if( kmty > 20) kmty = 0; 			
			}	
//			m_drawImages = TRUE;
			Invalidate(FALSE);
		...
		short e0, e2; //edge status
		case 'M':	// change original one KoMa to Horizontally Mirror One in KoMa mode
			if( m_drawMode ) return; // now only 0:KoMa mode, while 1:edgChain drawing mode 
			// avoid double operation in File-Menu onSave-aniMatedImages and Edit-Menu OnEditHSym()/Mirror 
			if( areaSet || animImage ) break; // == 1, drwNln == 2
			
			for( ikb = 0; ikb < m_komaNoX*m_komaNoY; ikb++) m_preKoM[ikb] = m_koM[ikb];// for Undo

			e0 = (m_koM[knap]>>0)%2 +((m_koM[knap]>>(4+0))%2) *2; // right edge status
			e2 = (m_koM[knap]>>2)%2 +((m_koM[knap]>>(4+2))%2) *2; // left
			m_koM[knap] = makeKoma( knap, 0, e2);// reverse edge status
			m_koM[knap] = makeKoma( knap, 2, e0);
			MessageBeep(MB_ICONEXCLAMATION); 	//MessageBeep(0);
			//drwNln =0;
			Invalidate(FALSE);
		break;
		...
		case 'G':// generation of (0) sequential and then Shift+: AutoGenration 
			...
			if( m_gemod == 0) // geneMode:sequential
			{
				...
				{			
					...
					m_strFileTitle = "KoMa-" + m_wndParam.m_ImageName; //for KoMaTile Mode
...
//					m_drawImages = TRUE;
			...
			} else if( m_gemod > 0 ) //geneMode:Rand V&H Sym, Rot Sym, Recursive
			{
				...
				for( ikb = 0; ikb < m_komaNoX*m_komaNoY; ikb++) m_preKoM[ikb] = m_koM[ikb];// for Undo
				pattnGenrtn(0);//Only KeyG sets Random   
...
//				m_drawImages = TRUE;
		...
		case 'X':	// back stroke , when the cursol is out of the Kolam area, make a negative black/White
			...
			{  			
				if( GetAsyncKeyState(VK_SHIFT) & 0x8000 ) 
				{	//(knap, -97, 0) delete the row,Cursor in last row except the last corner Koma  
					//(knap, -98, 0) delete the column, Cursor in last column except the last corner 
					//(knap, -99, 0) both row and column at the corner KoMa
					if( (knx ==0) & (kny ==0)) break;
					else if( (knx ==0) & (m_komaNoX == 1) )	connectDiscon( knap, -97, 0);
					else if( (kny ==0) & (m_komaNoY == 1) )	connectDiscon( knap, -98, 0);
					else if( (knx == m_komaNoX-1) & (kny == m_komaNoY-1) ) connectDiscon( knap, -99, 0); 
					else if( (knx == m_komaNoX-1) & (kny < m_komaNoY-1) )  connectDiscon( knap, -98, 0); 
					else if( (kny == m_komaNoY-1) & (knx < m_komaNoX-1) )  connectDiscon( knap, -97, 0); 
					Invalidate();//(FALSE)はImageSizeを変更した時　変更前の描画サイズの後が残る
				} else 
				{ 
				  	for( int ikb = 0; ikb < m_komaNoX*m_komaNoY; ikb++) m_preKoM[ikb] = m_koM[ikb];// for Undo	
					m_koM[knap] = short(0x8000); //=-1; //未完　diagonalも含めないといけない				
				}
			} else if( m_drawMode )// 1: for edgChain mode 
			{ 
			  // MessageBox(_T("here Key X and Shift+ for initialization"));
			  if( GetAsyncKeyState( VK_SHIFT ) & 0x8000 )// delete all fude
			  {   // set intialization
				memset(m_edgeChain, 0, sizeof(m_edgeChain) ); // intialization
				memset(m_numEdges, 0, sizeof(m_numEdges) ); // intialization
				memset(m_chnNo, 0, sizeof(m_chnNo) ); // intialization
			  ...
			  } else //only Key X deletes one Chain data
			  {
				...
				{  		 
					...
					{ // m_fudeNumのセット　 
						...
						else if( m_fudeNum > 0 &&  m_chnNo[m_fudeNum] == 0 ){ m_fudeNum--; }
					}
		...
		case 'Q':	// Q:Break edge-chain and then add one more edge-chain in same Fude
			if( m_drawMode )
			{	
				if( GetAsyncKeyState( VK_SHIFT ) & 0x8000 )// Key Shift+Q
				{	

					chnNum = m_chnNo[m_fudeNum] +1;
				...
				} else // only to break chain in same Fude
				{	
					m_chnNo[m_fudeNum]++; 
				}
				MessageBeep(MB_ICONEXCLAMATION); 	//MessageBeep(0);
		...
		case 'S':	//  twice Key to trace the Hitofude or to set the starting edge chain
			...
			{ 
				...
				{	// new starting point and Fude number keeping the old edge chains
					...
					{	
						...
						m_startChainX[m_fudeNum] = knap%m_komaNoX;
						m_startChainY[m_fudeNum] = knap/m_komaNoX;
						short udindx = 1- m_startSide[m_fudeNum]%2;// toggleSwitch LSB:1 CLOCKWISE, 0 UNCLOCK
				...
				{ 
				  ...
				  //	keyS_M = 0; 	  break;// avoind to restart from same starting point  
					...
					m_slpt = m_wndParam.m_SleepTime; //set again after moving mouse and speed adjasting
					MessageBeep(MB_ICONEXCLAMATION); 
					//getRepeatSequence(); //to get some repeating edgeChain sequences 
					//symMetry();	// check the symmetry 
				} //endof if( GetAsyncKeyState else
			...
			{	 
				...
				{	// starting the connection edge point of Koma m_koM[knap]
					...
					while(TRUE)	// set strtedg: the starting point LSB 2 bits:4 sides
						// LSB, 1:Up crossing & turns clockwise red(?)-color 0:Down & counterclockwise green(?)-color
					{	//　完　同じ駒のとき とSaveStillImageのとき　自動でEdge Directが変わる
					    if( !stilImage && (prvknap == knap) )// not SaveStillImage() or same KoMa 
						{
							strtedg ++; if(strtedg > 7) strtedg = 0; //alternately change  direction and edge
						}
						prvknap = knap; // previous knap
						if( ((m_koM[knap]>>strtedg/2) &1) || ((m_koM[knap]>>(4 +strtedg/2)) &1) )// m_koM[knap]の接続がある接続点で一筆開始する
						{
							prsntedg = strtedg/2;
							trce = 1; // index of first tracing
							break; 
						} 
					}//endof while  
...
//			if(prsntedg == -1){ break; } // not mouse edge pointing in the area
			...
			{  
				...
				{ 
						...
						m_wstr.Format("the last WebLoop was removed, now RgstredLoop=%3d", m_loopNum); 
				} else	m_wstr.Format("Already now NoRgstredLoop=%3d", m_loopNum);
			} else  
...
//					GetRValue(m_loopColor[m_loopNum-1]), GetGValue(m_loopColor[m_loopNum-1]), GetBValue(m_loopColor[m_loopNum-1]) ); 
				 ...
				 m_wstr.Format("RgstringLoop=%3d at startKoMa=%3d strtSide=%2d\n finalColorR=%3d ColorG=%3d ColorB=%3d",
...
static short knapb;	// knap backup in mouseMove()
...
void CChildView::OnMouseMove( UINT nFlags, CPoint point )
{
	...
	short centx = (short)(m_kks*(0.5+knx)) + offstx;
	short centy = (short)(m_kks*(0.5+kny)) + offsty;
...
// 　cursor out of the tile area Koma ground 下と右限界内活かす
	...
	{		    
		...
		if( prsntedg != prvedge ) Invalidate(FALSE); //changed edge from old one //refresh the new image
		return;
//	} else if( csrx < offstx + m_kks/4 || csry < offsty + m_kks/4 )
	...
	{ 		// x posion into the new neighbor koma, right || left side   
		...
		knap = (csrx - offstx)/m_kks +( (csry -offsty)/m_kks )*m_komaNoX; // new koma number
 		prvedge = prsntedg; //backup present edge
 		if( knapb != knap ) //when presentKoMa is different from PreviousKoMa
		{
...
//	} else	if( csrx > centx -m_kks*0.1 && csrx < centx +m_kks*0.1 && csry > centy -m_kks*0.1 && csry < centy +m_kks*0.1 )
	{	// around the pulli to sart tracing in KoMa mode//　点の周りのみでスタートさせる　2011/09/10 ver.4.5.1
		...
		//if( prsntedg != prvedge ) Invalidate(FALSE);//refresh the new image
		//上を除いてもちらつきと　勝ってにアニメを描くことは変わらない
		return;
...
void CChildView::OnLButtonUp( UINT nFlags, CPoint point )
{
	...
	if( m_drawMode )//	1: edgChain drawing mode 
	{	
		if( fnlimg ) //== 1  // reset KoMa mode finalimage at trce=3
		{			//	これがないとDialogを動かした後凍る　置く位置もKoMaMode if( !m_drawMode )にしても凍る
			fnlimg = 0; prsntedg = -1;
			Invalidate(FALSE);
			return;
		} else if( csrx > m_imageSiH || csry > m_imageSiV)
		{ 
			return; //cursor out of drawing Image  area   
...
//			if( m_fudeNum < 0 ) //どうしてか　これでは　音がする
			...
			} else // deleting at the cursor out as the same function as Key'X',Copy from Keyup'X'
			{ 
				if( m_numEdges[m_fudeNum][m_chnNo[m_fudeNum]] > 0) 
				{
					m_edgeChain[m_fudeNum][m_chnNo[m_fudeNum]] = m_edgeChain[m_fudeNum][m_chnNo[m_fudeNum]]>>1; //shift down 
		...
		{	//for edgeChain 32 bits
			m_chnNo[m_fudeNum]++;	
			m_numEdges[m_fudeNum][ m_chnNo[m_fudeNum] ] = 0; 
		} else if( m_numEdges[m_fudeNum][ m_chnNo[0] ] != 0 && knap == m_startChainX[m_fudeNum] +m_startChainY[m_fudeNum]*m_komaNoX )
		{		//cursol in the green dot starting Koma
			...
			if(prsntedg == -1){ 	// 未完　この処理を上のIf(に入れる　not mouse edge pointing in the area
					// nothing Dumy				// only start from same starting point  
			} else	m_startSide[m_fudeNum] = prsntedg*2 + udindx;// with Red edge of starting 4 edges x 2 directions, 4 bits: 4 sides,
	...
	{ 
		...
		if(  prsntedg == -100 ) return; //未完　これは上と同じく効いているか不明　cursor out of the window
		else if( fnlimg ) //== 1  
		{	//2009/11/28 reset of keeping finalline image at trce = 3
		...
		} else if( m_koM[knap] == 0 && prsntedg < 0){ 
			for( int ikb = 0; ikb < m_komaNoX*m_komaNoY; ikb++) m_preKoM[ikb] = m_koM[ikb];// for Undo
			m_koM[knap] = (short)0x8000; //delete 0 KoMa  
			Invalidate(FALSE);//
		...
		{								
				...
				{	// LSB, 1:Up crossing & turns clockwise red-color 0:Down & counterclockwise green-color
					//　完　同じ駒のとき とSaveStillImageのとき　自動でEdge Directが変わる
					if( !stilImage && (prvknap == knap) )// not SaveStillImage()
					{
...
//　これはChain全体についての相関係数、周期長　一方SymMety()は半分とか四分とかの特定長での相関係数を得る　
...
{ 
	bool edges[7400];	// X50xY37 KoMA x4 Edges =7400bits
	...
	for(short sl=1; sl < suNo; sl++)// sl: for shift phase/length　位相長
	{ 
		...
		{   
			...
			{ 
				if(  acef <= repCoeff[ir] && sl%repLength[ij] == 0 )// これまでの位相長の相関より低い && 周期性あり
				{  noc = 1;		break; }
			} // check a sequence of multiplex Length 
			if( noc == 1)  break; 
			else 
			{	
				if( acef > repCoeff[ir] )// これまでの位相長の相関より高い
				{
	...
	m_memo.Format("AllEdges%d MaxCor:%5.3f forshift(1=Cycle)Length%2d", suNo, repCoeff[0],repLength[0]); //for print to checkPe
	// to print m_memo in the Window in OnPaint() 
} //endof void CChildView::repeatSequence

//各筆毎に最大相関値のみを得て、表記出力する
//相関値はdouble小数点でなく％のshort整数記録とする 0.01=>1%と記録表記
...
void CChildView::symMetry() // checking autocorrelation with a half lenght with shift(sftL)
{
	...
	// 90degR-Sym beginning String 
	if( repLength[0] == suNo/4)	m_memo.Format("AllEdges%d Cor:%5.3fwithLen%2d,Swastika 90degRotated-Sym ", suNo, repCoeff[0],repLength[0]); //for print to check
	...
	{ 
		...
		{            //edges は　edges[suNo] であるのに 変数扱い？ 変数*eiがpointerAddress宣言あるのｄOK
			lgcA = *( ei +sftL);	
		...
		if( aCs == 1.0) // in SaveDataFile, print Symmetry at the number string from the start
		{
	...
	{ 
		...
		{				// 未完　判定が？180degRotatedなのにHVSymとでるなど逆もあり　m_komaNoX奇数、偶数の判定要素を除く
			//判定要素　軸の始点駒内か交差点上か、逆順相関(未完）で相関１か＝回転　など　駒総数奇数か偶数か　
			...
			//else if( repLength[0] == suNo/2 ) m_memo.Format("AllEdges%d 1:Co%5.3fwithLen%2d,180deg Rotated-Sym sftL%2d ", suNo, repCoeff[0],repLength[0], sftL); //for print to check
			//else if( repLength[0] == suNo/2 ) m_memo.Format("AllEdges%d 1:Co%5.3fwithLen%2d,V-H-SymAtEdgeString%2d axis? ", suNo, repCoeff[0],repLength[0], sftL); //for print to check
...
// Diagonalが混ざった複数筆で構成されるのは各相関値は要らない。
//　取りあえず特徴的な　相関値を各筆で記載する。
		m_symJudge[0].Format("test");
		m_memo.Format("# AllEdges%d Cor%5.3fwithShiftL%2d, %s", suNo, repCoeff[0],repLength[0], m_symJudge[0]);
...
void CChildView::preparingColor() //set color from ColorDailog
{
	...
	if( m_swell == 0) //default 
	{
		...
		m_tilColor = RGB(170, 110, 0);// すこしGroundに暗く近づけた
...
void CChildView::SetPatternArc(short centsign, short kxkysign, int nmedgn)
{ 	
	short sgx1,sgx2,sgy1,sgy2; short cnrx,cnry; short acentx, acenty;
...
*/	if( bAutog == TRUE && checkAll == 0)  return;//  checkAllをHeaderに入れたので　上の代わりになる
	...
	CPen skyPen(PS_SOLID, 4+m_kks/30, RGB(220, 220, 255)); // DiagonalLine
...
// dc.PlgBlt(plgpnts, &memDC, 0, 0, width, height, CBitmap(), 0, 0);
  ...
  //memDC.FillRect(CRect(0, 0, m_imageSiH, m_imageSiV), &CBrush(RGB(255, 0, 255))); // color background
	...
	// 効かない　m_slpt = m_wndParam.m_SleepTime; //set again after moving mouse
	...
	{	
		...
		short knx = kna%m_komaNoX;	//koma position number in x-axis
		short kny = kna/m_komaNoX;	//koma position number in Y-axis
		...
		{	                      //normal color print and night show color
			//if( m_koM[kna] > -1 ) //削除　紋様がなくても全てにタイル形状を示すため
			//{ 			
...
//OnPaint()の最後に移す　dc.BitBlt(0, 0, m_imageSiH, m_imageSiV, &memDC, 0, 0, SRCCOPY);// 

	// to draw the Pulli for Fude or Multi-Loops
	for(short nfu =0; nfu < m_fudeNum +1; nfu++)
	{	//完　m_fudeNum は本数でなく、0からの順序数であるので[最大m_fudeNum] できる
		...
		short knx = m_startChainX[nfu];	//koma position number in x-axis
		short kny = m_startChainY[nfu];	//koma position number in y-axis		
			centx = (short)(m_kks*(0.5+knx)) +offstx; //m_kks: Kolam Koma size pixel number in side
			centy = (short)(m_kks*(0.5+kny)) +offsty;
			//short priw = (pri*2);
		if( m_drawMode && (nfu == m_fudeNum) )//change color starting KoMa/Edge of最後の lastloop fude 
		{ 		//in Chain mode, show last Fude starting KoMa Pulli to do tracing
				memDC.SelectObject(&greenPen); memDC.SelectObject(&greenBrush);
				//short priw = (pri*2);
		...
		} else	if( m_loopNum >-1 || m_fudeNum >-1)
	...
	//2011/03/01 ここで　マウス位置の赤Dot表示をする 先のGreendDotに重ねるため　後にした　
...
//済み 最後のときのstarting edge　の赤バーをYellowバー　変える
	if (!trce )//== 0, trace 以外では太いEdgeバー＆Vertex丸を表示
	{
		...
		/* 		else if(m_drawMode && prsntedg == 10)	// ChainMode, to draw Blue right-bottom corner of red dot KoMa for two-point turning line
		{	現在Chainでの２点接続データを描かないので未使用
			memDC.SelectObject(&blue2Pen);	//Showing present Koma position 	
			memDC.SelectObject(&skyBrush);	//set color for filling for two-point connect
			short cnrxu = (short)(centpx+m_kks*0.40);
			short cnrxd = (short)(centpx+m_kks*0.60); 
			short cnryu = (short)(centpy+m_kks*0.40);
			short cnryd = (short)(centpy+m_kks*0.60);
	...
	{	 	  
	  ...
	  if( m_slpt > 0 && (m_komaNoX*m_komaNoY > 30) ) m_slpt =m_wndParam.m_SleepTime/2; else m_slpt = m_wndParam.m_SleepTime; 	
		//the same as  if( m_slpt > 0
	  ...
	  // 両モード消えない
	  //dc.TextOut( m_imageSiH,  m_imageSiV+60, "Here passed6");  

	  for( int nfu = 0; nfu < m_fudeNum +1; nfu++)// draw multi-fude(stroke)
	  {		//未完　m_fudeNumを直感と合うように本数とすべし　　　
			//m_fudeNum は既に本数よりー１である。検討済み　nfu = 0から始まるから　<m_fudeNum +1ではなく　< m_fudeNumではないか　
		//in Chain mode, show the starting direction stroke of each Fude
		//for set again Line-Color
				fnlLnPen.DeleteObject(); // 一旦破棄してからでないと再作成できない
		...
		// 今は使わない　short kkm; // = knx +kny*m_komaNoX, convert to KoMa oderNumber
		...
		{ 
		  ...
		  for( int nmch = 0; nmch < m_numEdges[nfu][nchn]; nmch++) // edge number in the chain 
		  {
			...
			{	// arc drawing 0: open= no side-connected edge with an arc stroke 
				...
				if( nchn == 0 && nmch == 0)	memDC.SelectObject(&redPenW); // for trace direction
			...
			{			       // 8th line for 45 deg, edge number 0:lower,1:left,2:upper,3:right
				if( nchn == 0 && nmch == 0)	// only starting edge
				{
					...
					y4 = (int)(centy + (m_kks/2 * sinpi8th[edgn*2 +1]) ); 
					if(udidx ==0) { x4 = x4 +(x3 -x4)/5; y4 = y4 +(y3 -y4)/5; } // for undercrossing without gray 
					memDC.SelectObject(&redPenW); // for trace direction
					memDC.MoveTo( x3,  y3); memDC.LineTo( x4,  y4); // set  starting point and then make a line
			...
			{	//除いた動き時間を乱数的に掛ける理由は?
				...
				Sleep(m_slpt);//Sleep( int(m_slpt * randf) ); //sleeping in msecond for animation
				dc.BitBlt(0, 0, m_imageSiH, m_imageSiV, &memDC, 0, 0, SRCCOPY);				
			}
			
			//neighbor KoMa
			edgn = edgn +2*udidx -1;// if( udidx ) edgn = edgn +1; else if( !udidx ) edgn = edgn -1; 
			if(edgn == 4)       edgn = 0; 
			else if(edgn == -1) edgn = 3;
...
//			}
	  ...
	  // 両モード消えない
	  //dc.TextOut( m_imageSiH,  m_imageSiV+50, "Here passed5");  
			if( GetAsyncKeyState('S') & 0x8000 || GetAsyncKeyState(VK_LBUTTON) & 0x8000)// force to stop drawing //VK_ESCAPE
			{ 
				keyS_M = 0; m_slpt = 0; cnvEc2KoMa = 0;  fnlimg = 1; prsntedg = -99;//  未完keeping the trace,　version 4.2.9 for stop OK
				dc.BitBlt(0, 0, m_imageSiH, m_imageSiV, &memDC, 0, 0, SRCCOPY);//
				return; 
	  ...
	  //これがなくとも　良いようだ。OnPaint()の最後に移す　dc.BitBlt(0, 0, m_imageSiH, m_imageSiV, &memDC, 0, 0, SRCCOPY);
	  if( stilImage == 1)
	  {
	...
	dc.BitBlt(0, 0, m_imageSiH, m_imageSiV, &memDC, 0, 0, SRCCOPY);// Ground Kolam Area
	//OnPaint の最後に移動。これは両Modeで必要　描画がなくともタイル等基本を表示する
	...
	//	 
	//ここでは消えるdc.TextOut( m_imageSiH,  m_imageSiV+20, "Here passed2");  
...
/* m_loopNum, loopNo, trce,   kna, finalcolor,  drawing
	...
	課題　現行カーソルからのLoop即表示とLoop記録全表示と重なる　どうするか？
		  ...
		  現行の自動切替が良いか　検討すべき　
*/
	//ここでは消える dc.TextOut( m_imageSiH,  m_imageSiV, "Here passed");  

	// draw KoMaTile MOde    
	short komaCo, udidx, passindx; 
	short loopNo = -1;
	short sttKoma, strtedgback;// set starting Koma for instead of knap(presnet Koma)
		// 未登録＝新規の筆輪のスタートEdgeが固定されている　なぜか？
	//	ここではChain NG KoMa OK
...
//for check proccess	 str.Format("1                    "); //
	 if(m_loopNum >0 ) 
	 {
		 trce = 2; //MultiLoopのとき　臨時 trce=1のアニメーションを省く
	 }
...
//		 dc.SelectObject( pOldPen ); //元のpenに戻す		 
		 ...
		 fnlLnPen.CreatePen(PS_SOLID, 4+m_kks/30, m_fnlColor);
		 pOldPen = dc.SelectObject(&fnlLnPen);//元のpenを覚えておく
	 } else if(trce == 2 && loopNo > -1 ) //set multi-loop drawing here 2011/05/01  to draw multi-loop
	 {	
		 m_slpt = m_wndParam.m_SleepTime; //set again after moving mouse
...
//												dc.SelectObject( pOldPen ); //元のpenに戻す		 
			...
			while(TRUE)
			{			// arcs in the same koma// set the Ellipse for Arc
...
//	CString strData; strData.Format("m_smoothmod=%d sced=%d nxtedg=%d edgn=%d",m_smoothmod,sced,nxtedg,edgn); AfxMessageBox(strData); //MessageBox for data	
			...
			{	// not work for stoping the trace,
				keyS_M = 0; m_slpt = 0; trce = 1; prsntedg = -99;	break; // 
			}  else if( prsntedg != -99 ) keyS_M = 0; // to avoid to restart at onKeyUp() with Key 'S' 
		...
		//if(m_slpt > 0 && m_komaNoX*m_komaNoY >30) m_slpt =m_slpt /2; else m_slpt = m_wndParam.m_SleepTime;
...
*/				}//endof if(edgn
		...
		if(stilImage == 1)// saving N-line still image
	...
	if( stilImage )//== 1 
	{
			...
			dc.TextOut( m_imageSiH, 0, str);  //the text is not drawn in the saved image, only on the window
	} else if( animImage ) //== 1,for saving an animation image
	{
			str.Format(" Key S:save Animation, %s-a000->nnn.jpg", m_strFileTitle);
			dc.TextOut( m_imageSiH, 0, str);  //the text is not drawn in the saved image, only on the window
	...
	{	// checking the hitofude one-string drawing succeed 
		...
		{ 	
			...
			dc.TextOut( m_imageSiH, 0, str); 
 									
			if( fnlimg ) //== 1 
			{    
				if( m_hitosccd )	str  = " Single Loop/Knot                         ";//== 1
				else			str.Format(" MultiLoop,Key W(web):RgstrAnother to %2d", m_loopNum);
            } else				str.Format(" Single or MultiLoop? Rgstred=%2d        ", m_loopNum);	 
			dc.TextOut( m_imageSiH, 20, str); 

			     if( m_gemod == 0)  str =" generate Sequence     ";
			else if( m_gemod == 1)	str =" generate H Symmetry   ";
			else if( m_gemod == 4)	str =" generate V Symmetry   ";
			else if( m_gemod == 2)	str =" generate Rot Symmetry ";
			else if( m_gemod == 3)	str =" generate Recursive    ";
			dc.TextOut( m_imageSiH, 40, str); 
		...
		{ 	 
			...
			{	// 保存モードではpimは初めの櫛型SingleまでのMultiを無視、計数されない。
				...
				if( m_saveDatafl ) //== 1 
				{
					...
					saveDatafile( str, m_strFileTitle);// ? strはPass名付でOK、m_strFileTitleには順番がないのになぜファイル名につくか？
				}   
			} // else 最後と一筆と別々に保存要   
			...
			{	
				...
				//{ //最後だから　OnSaveImagesData()でも Autogenerationのみの場合でもいずれも動作させる　
					//最後の発生数を記録する 画像に残る
				//ファイル名設定は用心しないと　同じ名前が既にあるときにRead不良で続行できず
				//strM = m_strPathFileName; // autoGnen のときは　saveDataflで既に最後にここをとおり初期化されている
				//if(strM == "") 
				//strM.Format("KolamImages\\KoMa-%s-All",m_wndParam.m_ImageName);
				strM.Format("KolamImages\\%s-All",m_strFileTitle);
				//else           strM = strM.Left( strM.ReverseFind( '.' ) );
				m_imageD_F_Name.Format("%s.jpg", strM);//incluing KolamImages
				memDC.SaveJPEG(m_imageD_F_Name); 
				strM.Format("%s-All",m_strFileTitle);
				str.Format("KolamImages\\%s-All.txt", m_strFileTitle);//なぜかstrM、str　いずれも-allが要？
				saveDatafile( str, strM);// (Pass名と活かすためのPassつきFile名, FileTile名のみ）
	...
	} else if( prsntedg == -100) //Mouse out of Window/Tile area in BothMode		
	{   }
	  // write chain data 
	  //if( (m_drawMode ) || (!m_drawMode) ) // && fnlimg&& fnlimg && prsntedg == -100 In Chain mode or KoMa Mode at Mouse out of Window/Tile area
		short strYp; 
		if( m_drawMode) strYp = 0; else strYp = 50; // Chain Mode else KoMa mode
		for(  nfu = 0; nfu < m_fudeNum +1; nfu++)//short nfu = 0; writeing all Chainedge Data on the screen
		{	// write multi-fude(stroke)			
			str.Format(" Fude%2d/Max10-Chain%2d/Max140-edges/Max32", m_fudeNum +1, m_chnNo[nfu] +1); 
			strYp =strYp +19; dc.		
			TextOut(  m_imageSiH, strYp, str);
			for( short nchn = 0; nchn < m_chnNo[nfu] +1; nchn++)
			{
					if(nchn ==10)
					{	str.Format(" neglecting Chains of 11-%2d", m_chnNo[0]+1); 
						strYp = strYp +19;		
						dc.TextOut(  m_imageSiH, strYp, str); break;
					} else 
					{	
						str.Format("%2d-%2d-%2d,", nfu +1, nchn +1, m_numEdges[nfu][nchn]); 
						 
						for(short nmch = m_numEdges[nfu][nchn] -1; nmch > -1; nmch--) // nfu +1 = order number from m_fudeNum +1
								str += (m_edgeChain[nfu][nchn] >>nmch) & 1? "1" : "0";
						str += "-";
						strYp = strYp +19;		
						dc.TextOut(  m_imageSiH, strYp, str);
					}
				} //endof for( nchn
		} //endof for( nfu
		//未完 このm_memo はChainModeでは最後Loop描画後で即書ける。Dialogで消える　KoMaModeとどう違うのか？
		// ChainModeでもChainDataがないときはこのm_memoも消えない。
		// KoMa ModeではStartして判定して書ける。Dialogで消えない。判定をどこでするか？ここか？
		//
		getRepeatSequence(); //to get some repeating edgeChain sequences 
		symMetry();
		dc.TextOut(  m_imageSiH, strYp+40, m_memo); // "here passed 4"data of repeatSequence() 
		//KoMaMode　で消えず、Chainで消える　入れても　消え方変わらず　dc.BitBlt(0, 0, m_imageSiH, m_imageSiV, &memDC, 0, 0, SRCCOPY);	
	//} //endof if( (m_drawMode)|| (fnlimg && !m_drawMode) )
	//} //endof if( prsntedg == -100				
	//dc.BitBlt(0, 0, m_imageSiH, m_imageSiV, &memDC, 0, 0, SRCCOPY);// これが無いと初期の絵がでない
	...
	//Invalidate(FALSE);// これをいれるとちらつくDialogの移動中に	
						//しかし、4.5.2　これをいれてある）でもちらつかない　別のことか？
	// Do not call in the original Why??
	// CWnd::OnPaint();  
...
void CChildView::OnEditPaste() 
{
	...
	//完　versiom 4.4.8 自動的にサイズ拡張　未完 DiagonalはBackupを要する
...
//			m_koMxy[na%m_komaNoX][na/m_komaNoX] = m_koM[na];
	...
	if( knxap +(LMeX -LMbX +1) > m_komaNoX )//横幅拡張
	{
...
void CChildView::OnEditHSym() 
{
	...
	// 現行　使用していない　keep old pattern in edtting matrix	
	for( short na = 0; na < m_komaNoX*m_komaNoY; na++)
				m_koMxy[na%m_komaNoX][na/m_komaNoX] = m_koM[na];
		
	for( short iy = 0; iy < LMeY -LMbY +1; iy++) //vertical
	{	
		for( short ix = 0; ix < (LMeX -LMbX +1)/2; ix++) //horizontal left half part
		{		
					short ie = LMeX -ix + (LMbY +iy) *m_komaNoX; // symmetric KoMa right half part
					m_koM[ ie ] = m_koM[LMbX +ix + (LMbY +iy) *m_komaNoX]; // copy  original KoMa to symmetric 
					//m_koM[ ie ] = m_koMxy[LMbX +ix][LMbY +iy];// copy  original KoMa to symmetric
					short e0 = (m_koM[ie]>>0)%2 +((m_koM[ie]>>(4+0))%2) *2; 
					short e2 = (m_koM[ie]>>2)%2 +((m_koM[ie]>>(4+2))%2) *2;
					m_koM[ie] = makeKoma( ie, 0, e2);// reverse edge status
					m_koM[ie] = makeKoma( ie, 2, e0);
 		} 
		if( (LMeX -LMbX +1)%2) //center column of odd Kolam matrix
		{
			short ie = LMbX +(LMeX -LMbX +1)/2 + (LMbY +iy) *m_komaNoX;
			short e2 = (m_koM[ie]>>2)%2 +((m_koM[ie]>>(4+2))%2) *2;
			m_koM[ie] = makeKoma( ie, 0, e2);// change only right edge, Keeping Left edge status
		}
	}// endof for(ii
	areaSet = 0; // index for edit Pasting using MouseL 
	Invalidate(FALSE);// show only the last image nagata
}// endof void CChildView::OnEditHSym()
...
void CChildView::OnEditUndo() 
{
	...
	areaSet = 0; //Reset index for edit Pasting using MouseL　 
	MessageBeep(MB_ICONEXCLAMATION); 	//MessageBeep(0);
//	m_drawImages = TRUE;
	Invalidate(FALSE);
...
void CChildView::OnEditMouseL() 
{
	// TODO: この位置にコマンド ハンドラ用のコードを追加してください
	if( m_drawMode ) return; 
	if( !Lb ) Lb = 9; //Lb== 0 index for Edit using Mouse_L
	else if(Lb == 1 || Lb == 9 || Lb == 10) 
	{
		Lb = 0;  areaSet = 0; drwNln = 0;// release 
	}
}
...
}//endof void CChildView::OnOpenKomaFile

//孫：GIFに対してのMemDCソースコードはないですが、WindowsのGDI+ APIはあります
void CChildView::OnSaveImageStill() //   Save StillImages
{
	// TODO: この位置にコマンド ハンドラ用のコードを追加してください
		MessageBeep(MB_ICONASTERISK); 	
		CreateDirectory("KolamImages", NULL);//2009/11/7 Holderを既存でないとき作成
		...
		if( !m_drawMode )	m_strFileTitle = "KoMa-" + fname; //for KoMaTile Mode
		else				m_strFileTitle = "Chain-" + fname;//for EdgeChain Mode

		//fnlimg = 0; // reset 2009/11/28 keeping the finalline image 
		stilImage = 1;	
		animImage = 0;
		drwNln = 0; // without N-lines
...
//		CString fname = strFileTtl; // m_paramDlg.m_ImageFileName;
		...
		fprintf( pf,"# ImageSize pixel, KoMaNumber X,Y, Image-Name,Date\n");
		fprintf( pf,"%10d, %2d,%2d, %20s,%6s\n", size, NoX, NoY, fname, m_cdate);
//2009/11/21 ColorData 		
		...
		{	// Chain mode
			CString str; 
			fprintf( pf,"# Edge ChainMode.This format is new since '13/06/10,'11/8/10,'09/09/30,'05/09/10/\n");// 先頭のEdgeは必要
			...
			fprintf( pf,"# LargestSequenceAutoCoEfficiency with shiftLength\n" );
			...
			{	// m_fudeNum は既にfude数よりー１である。検討済み　nfu = 0から始まるから　<m_fudeNum +1ではなく　< m_fudeNumではないか　
				...
				//改修 各筆毎に最大相関値、周期長を記載　以前の３大相関値を廃する。
				//未完　画面表示と合わせる　in symMetry()
// Diagonalが混ざった複数筆で構成されるのは各相関値は要らない。
//　取りあえず特徴的な　相関値を各筆で記載する。
				fprintf( pf,"# AllEdges%d Cor%5.3fwithShiftL%2d, %s\n", suNo, repCoeff[nfu],repLength[nfu], m_symJudge[nfu]);
			} // endof for( nfu
		    fprintf( pf,"#\n" ); 
...
// Save KoMa-Tile or Edge-Chain datafile in TXT format
...
{
	// TODO: この位置にコマンド ハンドラ用のコードを追加してください
		MessageBeep(MB_ICONASTERISK); 	//MessageBeep(0);
		CreateDirectory("KolamImages", NULL);//2009/11/7 Holderを既存でないとき作成
...
// for only KoMaTile Mode, save Autogenerated sequencially ImageName-n.jpg and also data.text
void CChildView::OnSaveImagesDatafiles() 
{
	...
	if( !m_drawMode ) 
	{
		...
		CString fname = m_wndParam.m_ImageName; //2009/11/29
		CString fnmdat;
		fnmdat= "KoMa-" + fname;	//only for KoMaTile Mode
		//ここにWindowで選択するようにする	//CFileDialog fileDlg(TRUE, NULL, NULL, OFN_HIDEREADONLY | OFN_NOCHANGEDIR, szFilter, this);
		CFileDialog fileDlg( FALSE, "txt", fnmdat, OFN_HIDEREADONLY | OFN_NOCHANGEDIR, szFilter, this /*AfxGetMainWnd()*/ );
		// if (fileDlg.DoModal() == IDOK) // コメントアウト
		if (HookFileDialogDoModal(&fileDlg) == IDOK)// 日本語->英語置換ファイルダイアログ
		...
		//{		以下必要か？　2011/03/11 元はあるが　Key Shift+Gのときと二重ではないか？	
			bAutog = TRUE; //set autogene sequential patterns and save image/data 
			pia = 0;	pim = 0;	pis = 0;	pio = 0;
...
// this code is only for Opening File in a Dialog to show txt/doc file 
// Open file HelpUsage.txt/doc and open only using "Open" of Mouse Right Menu 
static char BASED_CODE szFilter3[] = "Text&Doc File(*.txt *.doc) Only Mouse-RightMenu|*.txt;*.doc||";
void CChildView::OnHelpUsage()// in Menu Help/about  
{
	// TODO: この位置にコマンド ハンドラ用のコードを追加してください
	// Windowで選択するようにする
	CFileDialog fileDlg(TRUE, NULL, NULL, OFN_HIDEREADONLY | OFN_NOCHANGEDIR, szFilter3, this /*AfxGetMainWnd()*/);
	fileDlg.DoModal();
}
...
void CChildView::OnTimer(UINT nIDEvent) // SetTimer(10,
{
	...
	switch (nIDEvent)
	{
		case 10:
			...
			checkAll = 1;// checking if every KoMa > 0
			for( int inko = 0; inko < m_komaNoX*m_komaNoY; inko++) 
			...
			{	  
				...
				{		
					if( (m_koM[knap]>>strtedg/2) &1){ trce = 2; break; } //index to sencond tracing
					strtedg +=2;
				}//endof while  
// 				m_drawImages = TRUE;
